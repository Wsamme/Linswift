/**
 * Particles — WebGL 粒子动态背景
 * 来自 https://reactbits.dev/backgrounds/particles
 * 使用 ogl 进行 GPU 加速渲染
 */
...
const fragment = /* glsl */ `
  ...
  void main() {
    ...
    } else if(uAlphaParticles > 1.5) {
      // 镜片/气泡模式：边缘亮、中间透明
      if(d > 0.5) discard;
      float rim = smoothstep(0.3, 0.48, d);
      float inner = smoothstep(0.5, 0.35, d) * 0.08;
      float highlight = pow(smoothstep(0.5, 0.38, d), 3.0) * 0.5;
      float specular = smoothstep(0.22, 0.18, length(uv - vec2(0.35, 0.35))) * 0.4;
      float alpha = rim * 0.35 + inner + highlight + specular;
      vec3 col = vColor + 0.1 * sin(uv.yxx + uTime + vRandom.y * 6.28);
      gl_FragColor = vec4(col, alpha);
    } else {
      float circle = smoothstep(0.5, 0.2, d) * 0.7;
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uAlphaParticles: { value: lensMode ? 2 : (alphaParticles ? 1 : 0) },
      },
      transparent: true,
      depthTest: false,
    })
